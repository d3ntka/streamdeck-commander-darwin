    ...
    for button in buttons {
        match button {
            ...
            Button::Toggle { icon, on_icon, off_icon, .. } => {
                if let Some(i) = icon { icons.push(i.clone()); }
                if let Some(i) = on_icon { icons.push(i.clone()); }
                if let Some(i) = off_icon { icons.push(i.clone()); }
            }
        }
...
fn main() {
    ...
    generated.push_str("pub fn resolve_icon(icon_name: Option<&String>) -> Option<&'static str> {\n");
    ...
    generated.push_str("            tracing::warn!(\"Unknown icon style: {}, using filled:terminal\", style);\n");
